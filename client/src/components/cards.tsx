...
import DeleteRequest from "../req/delete"
...
const Cards = () => {
  ...
  const deleteHandler = async (id: number) => {
    const response = await DeleteRequest.deleteCollection(id)
    if (response) {
      setCards(
        produce(state => {
  ...
  return (
    <div class="flex flex-col gap-4">
      <form class="flex flex-col items-center gap-4 pb-4" onSubmit={async e => submitHandler(e)}>
      ...
      <For each={cards.cards}>
        {card => (
          <div class={`h-24 flex items-center rounded bg-white p-6 text-2xl ${card.japanese && "font-japanese"}`}>
            ...
            <div class="cursor-pointer" onClick={() => deleteHandler(card.id)}>
              <Cross />
            </div>
          </div>
        )}
